...
Node *CreateBST_Preorder(int pre[], int size, int mini, int maxi, int &i)
{
    //base case
    if(i >= size)
    {
    ...
    if(pre[i] > mini && pre[i] < maxi)
    {
        ...
        //recursive call
        root -> left = CreateBST_Preorder(pre, size, mini, root->data, i);
        root -> right = CreateBST_Preorder(pre, size, root->data, maxi, i);
...
Node* post_order(int pre[], int size)
{
    ...
    int index = 0;
    return CreateBST_Preorder(pre, size, INT_MIN, INT_MAX, index);
